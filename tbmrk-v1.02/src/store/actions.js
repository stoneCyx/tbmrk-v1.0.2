...
export const articleUserCollect = ({ commit },data) => {
    commit(types.ARTICLE_USER_STATUS, {status: 1});
    api.articleUserCollect(data).then(function (response) {
        const status = response.data.article.length == 10?0:2;
        commit(types.ARTICLE_USER, {
            list: response.data.article,
            status: status
...
export const addPhoto = ({ commit },data) => {
    api.addPhoto(data).then(function (response) {
        commit(types.PHOTO_ADD, {
            ...
            if (error.response) {
                showMsg({commit}, {
                    content: error.response.data.errorMsg || '上传失败',
                    type: 'danger'
                })
            }
        });
...
export const captcha = ({ commit }) => {
    api.captcha().then(function (response) {
            commit(types.CAPTHCHA, {
...
export const resendEmail = ({ commit },data) => {
    api.resendEmail(data).then(function (response) {
            commit(types.RESENDEMAIL, {
...
export const resetpwdByEmail = ({ commit },data) => {
    api.resetpwdByEmail(data).then(function (response) {
            commit(types.RESETPASSWORDBYEMAIL, {
                success: response.data
            })
            showMsg({ commit },{
                content: '修改密码成功，请用新密码登录！',
                type: 'info'
             })
        })
        .catch(function (error) {
